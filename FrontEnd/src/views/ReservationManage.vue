<template>
  <ion-page>
    ...
    <ion-content id="main-content" :fullscreen="true">
      <ion-card>
        <ion-card-header>
          <ion-card-title>나의 예약 목록</ion-card-title>
        </ion-card-header>
        <ion-grid>
          <ion-row>
...
async function getAllMyReservation(memberId) {
  try {
    const response = await axiosInstance.get(
      `/reservation/myReservation/${memberId}`
    );
...
async function btnDeleteAll() {
  console.log(myRevList.value.length);
  if (myRevList.value.length === 0) {
    const alert = await alertController.create({
      header: "안내",
